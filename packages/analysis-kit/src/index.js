...
export const CALLABLE_NODE_KINDS = new Set([
    "agent",
    "function",
    "human_gate",
    "route",
 ...
 */
export function hasQualifiedSuffix(name, pattern) {
    const match = pattern.exec(name);
    return match !== null && match.index > 0;
}
export function classifyDeclaration(facts) {
...
export function evidence(file, startLine, method, detail, confidence, symbol, endLine) {
    return { source: { file, startLine, endLine, symbol }, method, detail, confidence };
}
export function makeEdge(source, target, kind, itemEvidence, options = {}) {
    return {
...
export async function discoverSourceFiles(root, extensions, excludedFilePattern) {
    const found = [];
