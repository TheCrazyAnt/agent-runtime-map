...
export interface InitResult {
  ...
  /** What init did about keeping the generated map out of version control. */
  gitignore: GitignoreResult;
}

export interface GitignoreResult {
  file: string;
  /** The rule, derived from the configured outDir — never a hardcoded default. */
  rule: string;
  outcome: "created" | "appended" | "already-ignored";
}
...
export async function initContinuousProject(root: string): Promise<InitResult> {
  ...
  // The map is a build product: regenerated on every build, ~600KB with the
  // Viewer bundle inside it. Without this, the first `git add -A` after a local
  // build commits the whole thing, and the next one shows it as a diff.
  const outDir = typeof next.outDir === "string" && next.outDir.trim() ? next.outDir : defaults.outDir;
  const gitignore = await ignoreGeneratedOutput(resolvedRoot, outDir);
  return { configFile, created: !existing, addedKeys, suggestedScripts, gitignore };
}

/**
 * Makes sure the generated map is ignored, without disturbing anything else.
 *
 * An existing `.gitignore` is a file the project owns: this only ever appends,
 * and only when no existing rule already covers the directory. Nothing is
 * reordered, rewritten, or removed — a formatter that "tidied" someone's
 * ignore file would be a worse bug than the one it fixes.
 */
export async function ignoreGeneratedOutput(root: string, outDir: string): Promise<GitignoreResult> {
  const file = path.join(path.resolve(root), ".gitignore");
  // Written the way git reads a directory rule, from the configured outDir.
  const rule = `${outDir.replace(/^\.\//, "").replace(/\/+$/, "")}/`;
  ...
  if (existing === undefined) {
    await writeFile(file, `# Generated by agent-runtime-map: the map is rebuilt from source.\n${rule}\n`, "utf8");
    return { file, rule, outcome: "created" };
  }

  if (coversPath(existing, rule)) return { file, rule, outcome: "already-ignored" };

  // Append, preserving whatever the file already ends with.
  const separator = existing.length === 0 || existing.endsWith("\n") ? "" : "\n";
  const spacer = existing.trimEnd().length ? "\n" : "";
  await writeFile(
    file,
    `${existing}${separator}${spacer}# Generated by agent-runtime-map: the map is rebuilt from source.\n${rule}\n`,
    "utf8",
  );
  return { file, rule, outcome: "appended" };
}

/** Whether an existing rule already ignores this directory. */
function coversPath(contents: string, rule: string): boolean {
  const target = rule.replace(/\/$/, "");
  return contents.split(/\r?\n/).some((line) => {
    const entry = line.trim();
    // A comment says nothing, and a negation un-ignores rather than ignores.
