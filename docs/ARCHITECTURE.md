...
The Viewer owns presentation and deterministic playback state only. It fetches `graph.json` and, when available, `raw-graph.json`; computes one global layout with ELK; and highlights the selected `FeatureScenario` step by step. A double-click expands a logic node into a bounded local Raw Code Graph subgraph without re-laying out the global map. A focus control narrows the map to one step and everything below it, which is a different question from selecting a feature: a feature frames a route through the whole system and leaves the rest visible, while focus answers what one step is made of and hides everything else. A reader can select one of those children to inspect its own source range, with a breadcrumb back to the step, and can open exactly one further level through a control on the node; the depth cap is what keeps a single node from redrawing the whole call graph. The local server exposes a source-preview endpoint only for files already referenced by the Logic Graph, with path containment, allow-list validation, line limits, and size limits. Blueprint nodes, group frames, code-detail nodes, playback edges, edge appearances, and measurement helpers are isolated in `packages/react`, so an embedded viewer can consume the same visual contract without importing analyzer code. The Viewer does not execute the inspected project or claim that a live request is running. This keeps future Python, actual runtime trace, and model-assisted adapters compatible with the same UI.

## Adapters

Every adapter produces the same Raw Code Graph, and a project that mixes languages produces one map rather than one per language. What differs between adapters is parsing, never judgement: the classification rules — what an `agents/` directory means, when a name is a convention rather than a category, how much each signal is worth — live in `packages/analysis-kit` and are read by both. Two copies of those rules would drift, and the same repository would then read differently depending on which files it happened to contain.

The Python adapter delegates parsing to Python's own `ast` module through a bundled extractor script. A hand-rolled parser for an indentation-sensitive grammar would produce guesses dressed as facts, which is exactly what this graph must not contain; `ast.parse` builds a tree and does not import or execute the code it reads, so this does not breach the rule against executing a scanned project. The extractor reports structure only — definitions, calls, imports, decorators, string constants — and the TypeScript side decides what any of it means. Python's own shapes stop at that boundary: decorators, dunders, and `self` are read and discarded, so nothing Python-specific reaches the Viewer protocol. When no interpreter is available the adapter says so in a diagnostic, because silence would read as "this project has no Python logic".

## Trace bridge

A real run can be layered over the map through an optional event protocol. A `TraceEvent` names a stable id the graph already has — a logic node, a logic edge, or a raw node that is lifted to the step containing it — and says what happened there. It cannot introduce topology: an event matching nothing is returned as unmatched rather than drawn, and observation never alters a node's confidence or its evidence. Observed elements are styled unlike the statically inferred route, because the product's promise depends on a reader being able to tell an inference from an observation at a glance. This is a bridge, not a tracing system; replacing static facts with opaque spans would make the graph untraceable to source, which is the one thing it exists to guarantee.
